...
class Hangman(commands.Cog):
    ...
    async def hangman(self, ctx):
        ...
        embed = discord.Embed(
            title="Rules", description='Information on Hangman')
        embed.set_thumbnail(url=ctx.author.avatar_url)
        embed.add_field(name='`guess ***letter***',
                        value="Provide a letter to play")
        embed.add_field(name='`guess ***word***',
                        value="Provide whole word to solve", inline=False)
        ...
        if self.dictionaries[ctx.author.id]['self.own_word'] == True and self.check_message == 'own':
            ...
            for i in range(0, len(self.chosen_word)):
                if guess == self.chosen_word[i].lower():
                    self.guessed_letters = self.guessed_letters[:i] + \
                        self.chosen_word[i] + self.guessed_letters[i + 1:]
                    contains_guess = True
        ...
        # if player is doing random word
            ...
            for i in range(0, len(self.dictionaries[ctx.author.id]['self.chosen_word'])):
                if guess == self.dictionaries[ctx.author.id]['self.chosen_word'][i].lower():
                    self.dictionaries[ctx.author.id]['self.guessed_letters'] = self.dictionaries[ctx.author.id]['self.guessed_letters'][:i] + \
                        self.dictionaries[ctx.author.id]['self.chosen_word'][i] + \
                        self.dictionaries[ctx.author.id]['self.guessed_letters'][i + 1:]
                    contains_guess = True
            if not contains_guess:
    ...
    # if player is guessing their own word
        ...
        if not self.dictionaries[ctx.author.id]['self.has_ended']:
            ...
            for i in range(0, len(self.dictionaries[ctx.author.id]['self.guessed_letters'])):
                letters += '` ' + \
                    self.dictionaries[ctx.author.id]['self.guessed_letters'][i] + ' ` '
            status = '**{} guesses left** \n'.format(
                self.dictionaries[ctx.author.id]['self.remaining_guesses'])
            status += letters
