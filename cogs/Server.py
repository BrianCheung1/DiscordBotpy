...
class Server(commands.Cog):
    ...
    async def server(self, ctx):
        ...
        current_timezone_time = ctx.message.guild.created_at.replace(
            tzinfo=timezone('UTC')).astimezone(timezone('US/Eastern'))
        new_timezone_time = current_timezone_time.astimezone(
            timezone('US/Eastern'))

        # await ctx.channel.send("{0.name}\n{0.region}\n{0.icon_url}\n{0.member_count}\n{0.created_at}".format(ctx.author.guild))
        embed = discord.Embed(title=str(ctx.author.guild.name) + " Server",
                              description="Information On " + str(ctx.author.guild.name))
        embed.set_thumbnail(url=ctx.author.guild.icon_url)
        embed.add_field(name="Server Creation Date: ", value="` " + str(
            new_timezone_time.strftime("%A, %B %d %Y @ %H:%M:%S %p")) + " `", inline=False)
        embed.add_field(name="Region: ", value="` " +
                        str(ctx.author.guild.region) + " `", inline=False)
        embed.add_field(name="Total Users: ", value="` " +
                        str(ctx.author.guild.member_count) + " `", inline=False)
        embed.add_field(name="Online Users ", value="` " +
                        str(self.online_count) + " `", inline=True)
        embed.add_field(name="Offline Users ", value="` " +
                        str(self.offline_count) + " `", inline=True)
        embed.add_field(name="Bots ", value="` " +
                        str(self.bot_count) + " `", inline=True)
        embed.add_field(name="Categories ", value="` " +
                        str(self.categories) + " `", inline=True)
