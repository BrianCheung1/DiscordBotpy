...
class Blackjack(commands.Cog):
    ...
    async def leaderboard(self, ctx):
        """Provides balances of the top players"""
        embed = discord.Embed(
            title="Balance", description='Top Players Balance', colour=ctx.author.colour)
        with open('balance.txt') as f:
            ...
            for line in f:
                ...
                embed.add_field(name=str(x) + ". ` " + str(member) +
                                " `", value="Balance: $" + str(balance), inline=False)
                x += 1
                if x == 11:
                    break
    ...
    async def daily(self, ctx):
        """Provides users a daily balance of 500"""
        ...
        if self.is_player == True:
            with open('balance.txt') as f:
                for line in f:
                    ...
                    if str(balance[0]) == author:
                        ...
                        self.dictionaries[ctx.author.id]["self.money_balance"] = int(
                            balance[1])
            ...
            for line in reading_file:
                ...
                if str(balance[0]) == author:
                    ...
                    balance[1] = str(
                        self.dictionaries[ctx.author.id]["self.money_balance"])
        ...
        if self.is_player == False:
            ...
            author = str(ctx.author.id).replace(" ", "")
            f.write(
                author + ' ' + str(self.dictionaries[ctx.author.id]["self.money_balance"]) + "\n")
            f.close()
    ...
    async def bj(self, ctx, arg=100, members: discord.Member = None):
        ...
        self.dictionaries[ctx.author.id]["self.starting_hand"] = str(convertcards(
            self.dictionaries[ctx.author.id]["self.first_card"])) + " " + str(convertcards(self.dictionaries[ctx.author.id]["self.second_card"]))
        self.dictionaries[ctx.author.id]["self.dealers_starting_hand"] = str(
            convertcards(self.dictionaries[ctx.author.id]["self.dealers_first_card"]))
        ...
        self.dictionaries[ctx.author.id]["self.total"] += self.dictionaries[ctx.author.id]["self.first_card"] + \
            self.dictionaries[ctx.author.id]["self.second_card"]
        ...
        embed = discord.Embed(
            title="Rules", description='Information on Blackjack')
        embed.set_thumbnail(url=ctx.author.avatar_url)
        embed.add_field(
            name='`bj [amount]', value="Start a game of blackjack with bet amount, [Default Bet = $100]", inline=False)
        embed.add_field(
            name='`hit', value="Adds another card to your hand", inline=False)
        embed.add_field(
            name='`stand', value="Keep cards and let dealer play", inline=False)
        embed.add_field(name='`balance',
                        value="Show your balance", inline=False)
        embed.add_field(name='Win conditions',
                        value="Player higher total -> +$100\nDealer bust -> +$100\nPlayer Bust -> -$150\nDealer higher total -> -$100")
        await ctx.channel.send(content=None, embed=embed)

        if self.dictionaries[ctx.author.id]["self.total"] == 21:
            self.dictionaries[ctx.author.id]["self.money_balance"] += int(
                self.dictionaries[ctx.author.id]["self.bet"] * 1.5)
            changebalance(self, ctx)
            self.dictionaries[ctx.author.id]["self.busted"] = False
    ...
    async def stand(self, ctx):
        ...
        if self.dictionaries[ctx.author.id]["self.busted"] == False:
            while self.dictionaries[ctx.author.id]["self.dealers_total"] < 17:
                ...
                self.dictionaries[ctx.author.id]["self.dealers_starting_hand"] += " " + str(
                    convertcards(dealers_extra_card))
...
def hands(self, ctx):
    ...
    embed.add_field(name='` ' + ctx.author.name + ' Hand: ` ', value=str(self.dictionaries[ctx.author.id]["self.starting_hand"]) + "\n` Total: " + str(
        self.dictionaries[ctx.author.id]["self.total"]) + " ` ", inline=False)
    embed.add_field(name='` Dealers Hand: ` ', value=str(self.dictionaries[ctx.author.id]["self.dealers_starting_hand"]) + "\n` Total: " + str(
        self.dictionaries[ctx.author.id]["self.dealers_total"]) + " ` ", inline=False)
...
def convertcards(card):
    randomtencard = [':keycap_ten:', ':regional_indicator_j:',
...
def changebalance(self, ctx):
    ...
    for line in reading_file:
        ...
        if str(balance[0]) == author:
            ...
            balance[1] = str(self.dictionaries[ctx.author.id]
                             ["self.money_balance"])
            new_line = ' '.join(balance)
            new_file_content += new_line + "\n"
        else:
